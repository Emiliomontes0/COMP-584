...
namespace serverproject.Controllers
{
    ...
    public class SeedController(WorldCitySourceContext context, IHostEnvironment environment) : ControllerBase
    {
        ...
        public async Task<ActionResult> ImportCountriesAsync()
        {
            ...
            List<WorldCityDTO> records = csv.GetRecords<WorldCityDTO>().ToList();
            foreach (WorldCityDTO record in records)
            {
        ...
        public async Task<ActionResult>  ImportCitiesAsync()
        {
            ...
            using (CsvReader csv = new(reader, config))
            {
                IEnumerable<WorldCityDTO>? records = csv.GetRecords<WorldCityDTO>();
                foreach (WorldCityDTO record in records)
                {
